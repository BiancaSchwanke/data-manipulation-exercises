...
> **Nota Arpejo**: `scripts/brand_config.json` já traz um perfil de voz inferido
> do texto real do `.report` (analítico, ancorado em dado, sem hipérbole) —
> `brand_checker.py` carrega esse arquivo automaticamente. É uma primeira
> versão, não um guia de voz escrito pela Bianca — peça pra ela revisar as
> palavras banidas e os alvos de legibilidade (ajustados pra português, já
> que o Flesch Reading Ease original foi calibrado pra inglês) antes de
